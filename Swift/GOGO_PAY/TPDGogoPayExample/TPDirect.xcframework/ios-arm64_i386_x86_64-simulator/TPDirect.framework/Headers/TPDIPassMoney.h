...
#import "TPDIPassMoneyResult.h"

NS_ASSUME_NONNULL_BEGIN

@interface TPDIPassMoney : NSObject
...
/// Pass Payment Url to redirect to iPass Money App, complete iPass Money Transaction Get Result via Callback
/// - Parameters:
///   - url: Payment Url Get From Pay By Prime Success.
///   - callback: Get iPass Money Result, You Need to implement handleIPassMoneyUniversalLink in AppDelegate continueUserActivity
- (void)redirect:(NSString * _Nonnull)url completion:(void (^_Nonnull)(TPDIPassMoneyResult * _Nonnull result))callback;

/// Use this method to handle link come from TapPay ,
/// - Parameter link: From TapPay Payment Result Url
+ (BOOL)handleIPassMoneyUniversalLink: (NSURL * _Nonnull)link;

/// Set up Exception Observer
/// - Parameter aSelector: handle exception function.
+ (void)addExceptionObserver:(SEL)aSelector;

/// Use this function in your handle exception function to parse iPass Money result
/// - Parameter notification: Handle exception
+ (TPDIPassMoneyResult *)parseURL:(NSNotification *)notification;
